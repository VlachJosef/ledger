        ...
        describe "Node.Internal" $ do
            describe "addBlockToLedger" $ do
                it
                    ...
                    in addBlockToLedger block ledgerInitial `shouldBe`
                              NoValidTransaction
                    ...
                    let tx = mkTransaction testPk testPk2 10
                        ...
                        errorExpected =
                            InsufficientBalance
                                ...
                                120
                    in addBlockToLedger blockInitial ledgerInitial `shouldBe`
                              BlockToLedger
                                  [errorExpected]
                                  ledgerExpected
                                  blockExpected
            describe "applyTransactions" $
                it "should apply multiple transactions" $
                    ...
                    in applyTransactions ledgerInitial [tx, tx2] `shouldBe`
            ...
            describe "applyTransaction" $ do
                it
                    ...
                    in applyTransaction tx ([], emptyLedger, []) `shouldBe`
                              ([AddressNotFound address], emptyLedger, [])
                it "should add transaction into ledger" $
                    let tx = mkTransaction testPk testPk2 10
                        ledgerInitial = ledgerOf [(testPk, 100)]
                        ledgerExpected = ledgerOf [(testPk, 90), (testPk2, 10)]
                    in applyTransaction tx ([], ledgerInitial, []) `shouldBe`
                              ([], ledgerExpected, [tx])
                it
            ...
            describe "rewindBlock" $ do
              it "should rewind transaction" $
                ...
                in rewindBlock ledger block `shouldBe` ledgerExpected
